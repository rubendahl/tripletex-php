...
class ConsumerToken implements ModelInterface, ArrayAccess
{
    ...
    public function listInvalidProperties()
    {
        ...
        if (!is_null($this->container['token']) && (mb_strlen($this->container['token']) > 100)) {
            $invalidProperties[] = "invalid value for 'token', the character length must be smaller than or equal to 100.";
        }
    ...
    {
        if (!is_null($token) && (mb_strlen($token) > 100)) {
            throw new \InvalidArgumentException('invalid length for $token when calling ConsumerToken., must be smaller than or equal to 100.');
        }
